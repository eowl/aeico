...
describe('html()', () => {
  ...
  it('stores the callback as _cb', () => {
    const cb = (b: ElementBuilder) => { b.div() }
...
describe('render()', () => {
  ...
  it('appends elements from the template to the root', () => {
    render(html(({ div }) => { div({ textContent: 'hello' }) }), container)
    expect(container.children.length).to.equal(1)
    expect(container.firstElementChild?.textContent).to.equal('hello')
  })

  it('returns void', () => {
    const result = render(html(() => {}), container)
    expect(result).to.be.undefined
  })

  it('reuses the same builder instance across renders to the same root', () => {
    render(html(({ div }) => { div({ id: 'first' }) }), container)
    const el = container.firstElementChild

    render(html(({ div }) => { div({ id: 'first', className: 'updated' }) }), container)
  ...
  it('creates separate builder instances for different roots', () => {
    ...
    render(html(({ div }) => { div({ textContent: 'root1' }) }), container)
    render(html(({ div }) => { div({ textContent: 'root2' }) }), root2)
  ...
  it('passes an ElementBuilder to the callback', () => {
    ...
    render(html((b) => { captured = b }), container)
  ...
  it('supports shadow root as render target', () => {
    ...
    render(html(({ span }) => { span({ textContent: 'in shadow' }) }), shadow)
...
describe('getActiveBuilder()', () => {
  it('throws when called outside a render() context', () => {
    expect(() => getActiveBuilder()).to.throw('getActiveBuilder() called outside of a render() context.')
  })
...
describe('tags', () => {
  it('throws when accessed outside a render() context', () => {
    expect(() => (tags as any).div()).to.throw('getActiveBuilder() called outside of a render() context.')
  })
